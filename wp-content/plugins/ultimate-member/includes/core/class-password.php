		...
		function um_change_password_errors_hook( $args ) {
			...
			if ( UM()->options()->get( 'reset_require_strongpass' ) ) {
				...
				if ( strlen( utf8_decode( $args['user_password'] ) ) < 8 ) {
					UM()->form()->add_error('user_password', __('Sua senha deve conter no mínimo 8 caracteres','ultimate-member') );
				}

				if ( strlen( utf8_decode( $args['user_password'] ) ) > 30 ) {
					UM()->form()->add_error('user_password', __('Sua senha deve conter no máximo 30 caracteres','ultimate-member') );
				}

				if ( ! UM()->validation()->strong_pass( $args['user_password'] ) ) {
					UM()->form()->add_error('user_password', __('Sua senha deve conter no mínimo uma letra minúscula, uma letra maiúscula e um número','ultimate-member') );
				}
			...
			if ( isset( $args['user_password'] ) && isset( $args['confirm_user_password'] ) && $args['user_password'] != $args['confirm_user_password'] ) {
				UM()->form()->add_error('confirm_user_password', __('Senhas não são compatíveis','ultimate-member') );
			}
